...
def logistic_regression_with_gradient_descent(X, y, lr=0.0001, max_iterations=100000,tol=1e-7,damping=0.75,damping_interval=100):
    ...
    for iteration in range(max_iterations):
        ...
        if np.linalg.norm(gradient) < tol or np.sum(abs(w_new - w)) < tol:
            break
